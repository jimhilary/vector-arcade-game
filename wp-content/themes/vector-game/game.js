...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Use theme URL from WordPress (passed via wp_localize_script)
    const themeUrl = (typeof vectorGameTheme !== 'undefined' && vectorGameTheme.themeUrl) 
        ? vectorGameTheme.themeUrl 
        : window.location.origin + '/wp-content/themes/vector-game';
    ...
    function loadLeaderboard(filter = 'all') {
        ...
        if (typeof vectorGameAjax === 'undefined') {
            listEl.innerHTML = '<div class="loading">API not available. Activate plugin!</div>';
            return;
        }
        
        // Fetch scores
        fetch(vectorGameAjax.ajaxurl + '?action=vg_get_leaderboard&limit=20')
            .then(response => response.json())
            .then(data => {
                if (data.success && data.data.scores && data.data.scores.length > 0) {
                    let filteredScores = filterScores(data.data.scores, filter);
            ...
            .catch(error => {
                ...
                listEl.innerHTML = '<div class="loading">Failed to load leaderboard.</div>';
            });
    }
